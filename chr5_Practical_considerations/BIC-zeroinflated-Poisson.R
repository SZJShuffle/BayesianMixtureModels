...
## 1.load data 
nestsize = read.csv('nestsize.csv',stringsAsFactors = F,header = F)
X = nestsize$V1
...
##### compute the BIC for zif poisson model
d.zifpois=function(x,w,lambda) w*ifelse(x>0,0,1) + (1-w)*dpois(x,lambda)
## Computing BIC
BIC=0
for(i in 1:n){
  BIC = BIC - 2*log(d.zifpois(X[i],w,lambda))
}
BIC = BIC + ((KK-1) + 1)*log(n)  ### KK-1 independent weights, one variance, and KK means
BIC ## 1057

##### compute BIC for simple poisson model
BIC.pois=0
for(i in 1:n){
  BIC.pois = BIC.pois - 2*log(dpois(X[i],lambda=mean(X)))
}
mean(X)
BIC.pois = BIC.pois + log(n)  ### KK-1 independent weights, one variance, and KK means
BIC.pois ## 1267
