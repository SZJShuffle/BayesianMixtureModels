...
####### Semi-supervised Generative Model using EM solver #######
...
# first n belong to the training set, 
# and the rest belong to the test set
x = rbind(as.matrix(wine.training[,-1]), as.matrix(wine.test[,-1]))   
View(x)
...
## modeling the whole features as the multivariate gaussian.
...
while(!sw){
  ...
  ##Check convergence
  ## KLn can be viewed as our optimized goal (function in E-step)
  KLn = 0
  for(i in 1:(n+m)){
    for(k in 1:KK){
...
# qda
modqda = MASS::qda(grouping=wine.training[,1], x=wine.training[,-1], method="mle")
